...
import { getProgressTarget, isInGrowthMode } from "@/lib/gamification/growth-mode";
import type { Founder } from "@/types/database";

// The primary "game state" surface (SPEC §6) — progress bar is the
// centerpiece, XP/level/streak are secondary but always visible. Past 100
// customers this becomes an open-ended Growth Mode with stretch targets
// rather than a hard stop (SPEC §14).
export function GrowthHud({ founder }: { founder: Founder }) {
  const target = getProgressTarget(founder.current_customer_count);
  const growthMode = isInGrowthMode(founder.current_customer_count);
  const customerPct = Math.min(100, (founder.current_customer_count / target) * 100);
      ...
      <div>
        <div className="flex items-baseline justify-between">
          <p className="text-sm text-zinc-500 dark:text-zinc-400">
            {growthMode ? "Growth Mode — next target" : "Progress to 100 customers"}
          </p>
          <p className="text-sm font-medium text-black dark:text-zinc-50">
            {founder.current_customer_count} / {target}
          </p>
        </div>
