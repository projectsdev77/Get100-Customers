...
| LLM (AI coaching engine) | Gemini API free tier (Google AI Studio) during dev — confirmed provider per SPEC §18/docs/10 | Gemini API paid tier at production rate limits/quotas | Free tier is rate-limited (**20 requests/day per model**, confirmed live — much tighter than a typical rate cap) — fine for occasional manual testing, not for real concurrent founders. It also has a **hard `limit: 0` quota on pro-tier models**, so `src/lib/ai/gemini.ts`'s "capable" tier currently points at the same flash model as "fast" rather than a real pro model — swap it to one once billing is enabled. `src/lib/ai/generate-structured.ts` optionally falls back to Groq's free tier (~1,000 req/day) when Gemini's quota is hit — set `GROQ_API_KEY` (SETUP.md §4) to enable it; the app works fine without it, Groq is a stopgap for dev-time quota pain, not a launch dependency |
